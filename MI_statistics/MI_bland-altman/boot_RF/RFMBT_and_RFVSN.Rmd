...
<!-- version -->
<div align=right>
<font style="color:#26a169; font-size:80%">Processed: 2024.08.30 13:48</font>
</div>
...
Decision by 95% confidence interval:
	avg{RFVSN - RFMBT} = -0.0371 + -0.9145 {RFMBT - mean(RFMBT)}

	H0: accuracy point(0,0) inside 95% CI [-0.1018, 0.0262] is not rejected
	n: 16 data pairs
	bootstrapping: 2000 resamplings
	Bias 95%CI: between -0.1018 and 0.0262.
...
Decision by 95% confidence band:
	avg{RFVSN - RFMBT} = -0.3735 + 0.4306 {(RFMBT + RFVSN)/2}

	H0: precision line horizontal (x,0)
	with translation from -0.1018 to 0.0262 inside 95% CB is not rejected
	n: 16 data pairs
	bootstrapping: 2000 resamplings
</pre>
```{r, echo=FALSE, out.width = "90%"}
...
Decision by 95% confidence band:
	avg{True[RFVSN]} = 0.4371 + 0.3915 {True[RFMBT]}
...
Decision by 95% confidence ellipse:
	...
	H0: intercept = [-0.1018,0.0262], slope = 1 inside 95% ellipse is not rejected
	n: 16 data pairs
	bootstrapping: 2000 resamplings
</pre>
```{r, echo=FALSE, out.width = "90%"}
